...
if __name__ == '__main__':
    ...
    # prepare dataset
    # use the dataset without augmentation to split training and validation samples
    ...
    # make sure that patches from each case will not occur in both training and validation set at the same time
    ds_df = pd.read_excel(df_path, sheet_name="Australian new")
    group = ds_df.GROUP  # patches from the same case were annotated in the same group
    ...
    for seed in seeds:
        ...
        for fold, (train_indices, test_indices) in enumerate(gkf.split(data, data_label, group)):
            ...
            # Define the data loaders for the train and test sets (to include the augmented patches for the training set)
            ...
            for i in train_indices:
                if i <= 34:
                    syd_diff_index = [i,35+i*7,35+i*7+1,35+i*7+2,35+i*7+3,35+i*7+4,35+i*7+5,35+i*7+6]
                    sydney_augmented_diff_indice.extend(syd_diff_index)
                if 34<i<=38:
                    hk_diff_index = [i+245,284+(i-35)*7,284+(i-35)*7+1,284+(i-35)*7+2,284+(i-35)*7+3,284+(i-35)*7+4,284+(i-35)*7+5,284+(i-35)*7+6]
            ...
            # train_loader = DataLoader(MDataset(data, data_label, data_transforms['train']), batch_size=batch_size,
            #                           sampler=train_sampler)
            # val_loader = DataLoader(MDataset(data, data_label, data_transforms['val']), batch_size=batch_size,
            #                         sampler=test_sampler)
            # dataloaders = {'train': train_loader, 'val': val_loader}
            ...
            image_name = f"/Users/jessy/Documents/PycharmProjects/Combined patch/Performance of fold {fold+1}"
            fig.savefig(image_name)
            # save the model
            save_path = f'/Users/jessy/Documents/PycharmProjects/Combined patch/model-fold-{fold+1}.pth'
