...
if __name__ == '__main__':
    ...
    data_dir = "/Users/jessy/Documents/PycharmProjects/Combined patch/Chinese"
    n = 480

    ds_df = pd.read_excel(df_path, sheet_name="Chinese new")
    group = ds_df.GROUP
    ...
    for seed in seeds:
        ...
        for fold, (train_indices, test_indices) in enumerate(gkf.split(data, data_label, group)):
            ...
            for i in train_indices:
                if i <= 29:
                    syd_diff_index = [i,30+i*7,30+i*7+1,30+i*7+2,30+i*7+3,30+i*7+4,30+i*7+5,30+i*7+6]
                    sydney_augmented_diff_indice.extend(syd_diff_index)
                if 29<i<=59:
                    hk_diff_index = [i+210,270+(i-30)*7,270+(i-30)*7+1,270+(i-30)*7+2,270+(i-30)*7+3,270+(i-30)*7+4,270+(i-30)*7+5,270+(i-30)*7+6]
            ...
            for j in train_indices:
                ...
                if 59 >= j > 29:
                    test_indices_new.append(j+210)
                else:
                    test_indices_new.append(j + 420)
            sorted_test_indices_new = natsorted(test_indices_new)

            # print(sorted_new_train_indices)
            # print("**" * 50)
            # prepare dataset
            data_dir_ = "/Users/jessy/Documents/PycharmProjects/Combined patch/augmented/Chinese"
            m = 901
